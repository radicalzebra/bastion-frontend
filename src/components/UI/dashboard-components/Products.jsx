...
function Products(props) {
    ...
    const {products,loading,setFilter,setSearch} = useGetSellerProducts()
  ...
  return (
       ...
       <main className={`flex gap-10 ${props.className} overflow-x-hidden`}>
           ...
           <Card className={`flex flex-col  gap-5 w-full pr-8 relative `}>
                 ...
                 <section className='flex flex-col gap-12  text-black my-5'>

                        <section className='flex gap-4 bg-red- justify-center w-full'>
                           
                             
                             <div className='flex items-center w-2/6 gap-5  bg-gray-50 rounded-xl  border-2 px-2 py-2'>
                             ...
                             <select onChange={(e) => setFilter(e.target.value)} name="sortBy" className='bg-gray-100 outline-none px-2 pr-5 rounded-xl '>
