  ...
  return (
    <section className={`${className}`}>
        <GeneralHeading  name={"name"} heading="Product Name" >
            <input value={obj.name} onChange={(e)=> onChangeHandler(e.target.value,"name") }  className='bg-white border-2 rounded-md h-9 px-2 outline-none w-2/3' maxLength="20"  placeholder="Nike Air Force" />
        </GeneralHeading>

         <GeneralHeading  name={"price"} heading="Price" >
            <LimitedNumInput value={obj.price} name={"price"} onChangeHandler={onChangeHandler} className={"bg-white border-2 rounded-md h-9 px-2 outline-none w-2/3"} max={10000} placeholder={"$"}/>
         </GeneralHeading>
