...
import itemPresent from './../Utilities/CheckCartItemPresence';
...
export const ProdCard = (props) =>{
  ...
  // console.log(cartTouched)

  useEffect(()=>{
    if(logged) setCartTouched(itemPresent(props.id,user.cart))
  },[logged])
  ...
  const onCartBtnClick = () => {
    ...
    if(logged) {
        if (cartTouched) {
            ...
            dispatch(updateCart(props.id,user.cart,"remove"))
            setCartTouched(false);
        } else {
            // dispatch(cartActions.addToCart(props.id));
            dispatch(updateCart(props.id,user.cart,"add"))
            setCartTouched(true);
        }
    }
      ...
      <Card className = {`flex flex-col gap-2 h-98 bg-red-90 rounded-lg text-black  ${props.className}`}>
        ...
        <figure className="relative w-72 h-3/4 rounded-lg overflow-hidde ">
           ...
           {!props.ranking && <div onClick={onCartBtnClick} className='bg-white absolute -bottom-2 -right-2 p-3 rounded-full '>
               <div  className={`p-2 ${cartTouched && " bg-gray-950"} bg-red-600 rounded-full hover:cursor-pointer hover:scale-125 transition-all `}>
