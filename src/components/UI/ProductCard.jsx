  ...
  return (
      <Card className = {`flex flex-col gap-2 h-98 bg-red-90 rounded-lg text-black  ${props.className}`}>
        ...
        <figure className="relative w-72 h-3/4 rounded-lg overflow-hidde ">
           <img src={hero} onClick={()=> navigate(`/product/${props?.category}`)} className='w-full h-full rounded-lg hover:cursor-pointer hover:brightness-90' alt="" />
           {/* <div className='bg-black absolute bottom-0 -right-0 rounded-l-full translate-x-1/2  translate-y-1/2   w-36 h-36'></div> */}
           <div className='bg-white absolute -bottom-2 -right-2 p-3 rounded-full '>
              <div className={" p-2 bg-red-600 rounded-full hover:cursor-pointer hover:scale-125 transition-all"}>
        ...
        <section className='flex flex-col h-1/4 px-2 justify-around rounded-lg'>
          <div className='flex flex-col gap-1'>
             <ProductCardRating rating="4.8" numReviews="742"/>
             <p onClick={()=> navigate(`/product/${props?.category}`)} className='text-sm font-semibold hover:cursor-pointer hover:text-gray-400'>Men AIR MAX SOLO Sneakers</p>
          </div>

          <span className='font-semibold text-sm text-yellow-5 px-2 rounded-full w-fit bg-yellow-200'>$444</span>
        </section>
